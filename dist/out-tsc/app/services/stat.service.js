...
var StatService = (function () {
    ...
    StatService.prototype.addStatToGame = function (data) {
        var headers = new Headers({ 'Content-Type': 'application/json' });
        var options = new RequestOptions({ headers: headers });
        var _path = environment.serverProtocol + environment.serverUrl + ':' + environment.serverPort + '/games/stat';
        return this._http.post(_path, data, options)
            .map(this.extractData)
            .catch(this.handleError);
    };
    StatService.prototype.addStatToPlayer = function (data) {
        var headers = new Headers({ 'Content-Type': 'application/json' });
        var options = new RequestOptions({ headers: headers });
        var _path = environment.serverProtocol + environment.serverUrl + ':' + environment.serverPort + '/players/stat';
        return this._http.post(_path, data, options)
            .map(this.extractData)
            .catch(this.handleError);
    };
    StatService.prototype.addScore = function (data) {
        var headers = new Headers({ 'Content-Type': 'application/json' });
        var options = new RequestOptions({ headers: headers });
        var _path = environment.serverProtocol + environment.serverUrl + ':' + environment.serverPort + '/games/stat/score';
        return this._http.post(_path, data, options)
            .map(this.extractData)
            .catch(this.handleError);
    };
